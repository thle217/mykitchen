...
let getCategoryById = async (req, res) => {
  ...
  let createCategory = async (req, res) => {
    const sql1 = `select * from categories where category_name ='${req.body.category_name}'`;
    ...
    if (caterogy.length > 0) {
      ...
      if (req.body.category_name !== undefined) {
        const sql = `INSERT INTO categories VALUES (DEFAULT,'${req.body.category_name}')`;
  ...
  let updateCaterogy = async (req, res) => {
    ...
    if (categorys.length === 0) {
      res.status(404).json({ result: "Category does not exist" });
        ...
        const sql =`update categories set category_name= '${req.body.category_name===""?categorys[0].category_name:req.body.category_name}' where category_id='${req.params.id}'`;
